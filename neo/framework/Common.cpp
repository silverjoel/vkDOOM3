...
CONSOLE_COMMAND( find, "search all exposed system data for a given string", NULL ) {
	if ( args.Argc() < 2 ) {
		idLib::Printf( "Usage: find <search string>\n" );
		return;
	}

	idStr search = args.Argv( 1 );
	idStrList found;

	// search cvars
	const int numCVars = cvarSystem->Num();
	for ( int i = 0; i < numCVars; ++i ) {
		const idCVar * cvar = cvarSystem->FindByIndex( i );
		if ( cvar == NULL ) {
			continue;
		}

		if ( idStr::FindText( cvar->GetName(), search.c_str(), false ) == idStr::INVALID_POSITION ) {
			continue;
		}

		idStr entry;
		entry.Format( "^4[^2cvar^4] ^0%s", cvar->GetName() );
		do {
			entry.Append( ' ' );
		} while ( entry.Length() < 40 );
		entry.Append( cvar->GetDescription() );
		entry.Append( "\n" );
		found.Append( entry );
	}
	...
	// search commands
	idStrList cmds;
	cmdSystem->FindCommands( search.c_str(), cmds );
	const int numCmds = cmds.Num();
	for ( int i = 0; i < numCmds; ++i ) {
		idStr entry;
		entry.Format( "^5[^2command^5] ^0%s", cmds[ i ].c_str() );
		do {
			entry.Append( ' ' );
		} while ( entry.Length() < 40 );
		entry.Append( cmdSystem->GetCommandDescription( cmds[ i ].c_str() ) );
		entry.Append( "\n" );
		found.Append( entry );
	}

	found.SortWithTemplate( idSort_Str() );

	if ( found.Num() > 0 ) {
		idLib::Printf( "Found %d item(s):\n\n", found.Num() );
		for ( int i = 0; i < found.Num(); ++i ) {
			idLib::Printf( found[ i ].c_str() );
		}
	} else {
		idLib::Printf( "^1No items found^0\n" );
	}
}
...
CONSOLE_COMMAND( writePrecache, "writes precache commands", NULL ) {
	if ( args.Argc() != 2 ) {
		idLib::Printf( "USAGE: writePrecache <execFile>\n" );
		return;
	}
