		...
		while( index < end ) {
			...
			switch( command.type ) {
				...
				case FC_START_EMITTER: {
					int stridx = command.string->Find( " " );
					if ( stridx >= 0 ) {
						idStr name = command.string->Left( stridx );
						idStr particle = command.string->Right( command.string->Length() - stridx - 1 );
...
bool idDeclModelDef::Parse( const char *text, const int textLength, bool allowBinaryVersion ) {
	...
	while( 1 ) {
		...
		if ( token == "inherit" ) {
			...
			if ( !copy ) {
				idLib::Warning( "Unknown model definition '%s'", token2.c_str() );
			} else if ( copy->GetState() == DS_DEFAULTED ) {
				idLib::Warning( "inherited model definition '%s' defaulted", token2.c_str() );
				MakeDefault();
				return false;
			} else {
				CopyDecl( copy );
